import { year, shortYear } from './constants';
import { gitHubUrls, pad } from './utils';

export type User = {
    name: string;
    lang: (day: number) => string;
    langName: string;
    encoding?: (day: number) => string;
    solutionUrl: (day: number, part: number) => string;
    solutionWebUrl: (day: number, part: number) => string;
};
